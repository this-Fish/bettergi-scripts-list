(async function () {
    ...
    function validateAndSetDefaults(specifyHours, specifyMinutes) {
        if (isNaN(specifyHours) || specifyHours > 23) {
            if (specifyHours > 23) {
                log.warn("设置指定時間錯誤，請使用 0~23 時，将使用默认值：4時");
            } else {
                log.warn("你没有设置指定時，将使用默认值：4時");
            }
            specifyHours = defaultHours;
        }
        if (isNaN(specifyMinutes) || specifyMinutes > 59) {
            if (specifyMinutes > 59) {
                log.warn("设置指定時間錯誤，請使用 0~59 分，将使用默认值：0分");
            } else {
                log.warn("你没有设置指定分鐘，将使用默认值：0分");
            }
    ...
    log.info(`等待 ${Math.floor((timeUntilNextTime / 60000/60))} 小時 ${(timeUntilNextTime / 60000%60).toFixed(0)} 分 ，直到下一個 ${validatedHours} : ${validatedMinutes}`);
    // 多等待10秒
    await sleep(timeUntilNextTime + 10000);
    log.info(`時間到了！現在是 ${specifyHours}：${specifyMinutes}`);
